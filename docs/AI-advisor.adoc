...
----
graph TD
    A[Raspberry Pi / Sensor Nodes] -->|Telemetry via UDP/MQTT| B[Central AI Node]
    B -->|Model Inference & Correlation| C[Anomaly Reports / Event Summaries]
    C --> D[Distributed Storage or Cloud Sync (optional)]
----
...
The AI node runs a lightweight inference server based on **LLaMA-compatible architectures**, deployed via `llama.cpp` or `text-generation-webui` with quantized models (3B–7B).  
